...
if ( empty( $course_id ) ) :
	...
	<ul>
		...
		foreach ( $courses as $course ) :
			if ( $course->day == 99 && $course->date < date( 'Y-m-d' ) )
				continue;
			?>
			<li><a href="<?= add_query_arg( 'course_id', $course->id, get_permalink( $post_id ) ) ?>">
				<?php
				if ( $course->day == 99 ) :
				...
				echo cbs_get_time_formatted( $course->start, $course->end );
				?>
			</a></li>
		<?php endforeach; ?>
	</ul>
	...
	<?php if ( count( $courses ) == 0 ) : ?>
		<p><?php _e( 'Unfortunately, no course times are currently available.', 'course-booking-system' ); ?></p>
	<?php
	endif;
...
foreach ( $courses as $course ) {
	if ( $course->day == 99 ) :
		...
		echo '<h2>'.get_the_title( $post_id ).', <span class="weekday">'.date_i18n( $date_format, strtotime( $course->date ) ).', </span>'.cbs_get_time_formatted( $course->start, $course->end ).'</h2>';
	else :
		$weekday = strtotime( 'Sunday +'.$course->day.' days' );
		echo '<h2>'.get_the_title( $post_id ).', <span class="weekday">'.date_i18n( 'l', $weekday ).', </span>'.cbs_get_time_formatted( $course->start, $course->end ).'</h2>';
	endif;

	echo '<h4>'; echo ( $free ) ? __( 'Free', 'woocommerce' ) : cbs_get_price_level_name( $price_level ); echo '</h4>';
	...
	elseif ( ( !empty( $course->date ) && $course->date > date( 'Y-m-d' ) ) ) :
		...
		if ( in_array( reset( $current_user->roles ), $roles ) && $booking_in_advance > 1 ) {
			if ( empty( $start_date ) ) {
	...
	if ( $course->day != 99 && $booking_in_advance > 1 ) // No slider if only one possible day to book
	...
	for ( $i = strtotime( strtolower( date( 'l', $weekday ) ), strtotime( $start_date ) ); $i <= strtotime( $booking_in_advance_date ); $i = strtotime( '+1 week', $i ) ) {
		...
		// if ( ( $date != date( 'Y-m-d' ) || $current_time < strtotime( $date.' '.$course->end ) ) || in_array( reset( $current_user->roles ), $roles ) ) {
		if ( $date >= date( 'Y-m-d' ) || in_array( reset( $current_user->roles ), $roles ) ) {
			echo '<div class="slide">';
				...
				if ( is_user_logged_in() && ( current_user_can( 'administrator' ) || current_user_can( 'author' ) ) ) {
					...
					<form action="#" method="POST">
						...
						<p class="trainer"><?php _e( 'Trainer', 'course-booking-system' ); ?>: <strong><?= $user_info->display_name ?></strong><br>
							...
							<select name="user_id" onchange="cbs_action_substitute( <?= $course_id ?>, '<?= $date ?>', this.options[this.selectedIndex].value );">
								...
								<?php
								$admins = get_users( array( 'role__in' => $roles ) );
				...
				foreach ( $abos as $abo ) {
					...
					if ( $abo_start <= $date && $abo_expire >= $date && !in_array( $date, $abo_alternate ) ) {
						...
						if ( strtotime( $user_info->user_registered ) > strtotime( $date.' -4 weeks' ) )
				...
				foreach ( $bookings as $booking ) {
					...
					$table .= '<tr>';
						...
						if ( $price_level_for_lower_course && $price_level != $price_level_card && !$free )
							$description .= ' ('.cbs_get_price_level_name( $price_level_card ).')';
				...
				if ( ( $show_availability || in_array( reset( $current_user->roles ), $roles ) ) && !( $date >= $holiday_start && $date <= $holiday_end ) && !cbs_is_holiday( date( 'd', $i ), date( 'm', $i ), date( 'Y', $i ) ) ) {
					if ( $attendance_count >= $attendance ) {
						?>
						<p class="availability"><?php _e( 'Availability:', 'course-booking-system' ); ?> <strong class="red">0</strong><?php if ( in_array( reset( $current_user->roles ), $roles ) ) : ?> | <small><a href="#" title="<?php _e( 'Increase the number of participants by 1 for this day only.', 'course-booking-system' ); ?>" class="action-attendance" data-id="<?= $course_id ?>" data-date="<?= $date ?>" data-user="<?= $user_id ?>" data-attendance="<?= $attendance + 1 ?>">+ 1</a> <?php _e( 'or', 'course-booking-system' ); ?> <a href="#" title="<?php _e( 'Decrease the number of participants by 1 for this day only.', 'course-booking-system' ); ?>" class="action-attendance" data-id="<?= $course_id ?>" data-date="<?= $date ?>" data-user="<?= $user_id ?>" data-attendance="<?= $attendance - 1 ?>">- 1</a></small><?php endif; ?></p>
						...
						?>
						<p class="availability"><?php _e( 'Availability:', 'course-booking-system' ); ?> <strong class="yellow"><?= ( intval( $attendance ) - intval( $attendance_count ) ) ?></strong><?php if ( in_array( reset( $current_user->roles ), $roles ) ) : ?> | <small><a href="#" title="<?php _e( 'Increase the number of participants by 1 for this day only.', 'course-booking-system' ); ?>" class="action-attendance" data-id="<?= $course_id ?>" data-date="<?= $date ?>" data-user="<?= $user_id ?>" data-attendance="<?= $attendance + 1 ?>">+ 1</a> <?php _e( 'or', 'course-booking-system' ); ?> <a href="#" title="<?php _e( 'Decrease the number of participants by 1 for this day only.', 'course-booking-system' ); ?>" class="action-attendance" data-id="<?= $course_id ?>" data-date="<?= $date ?>" data-user="<?= $user_id ?>" data-attendance="<?= $attendance - 1 ?>">- 1</a></small><?php endif; ?></p>
						...
						if ( ( intval( $attendance ) - intval( $attendance_count ) ) > 5 ) {
							?>
							<p class="availability"><?php _e( 'Availability:', 'course-booking-system' ); ?> <strong><?= sprintf( __( 'more than 5', 'course-booking-system' ), ( intval( $attendance ) - intval( $attendance_count ) ) ) ?></strong><?php if ( in_array( reset( $current_user->roles ), $roles ) ) : ?> | <small><a href="#" title="<?php _e( 'Increase the number of participants by 1 for this day only.', 'course-booking-system' ); ?>" class="action-attendance" data-id="<?= $course_id ?>" data-date="<?= $date ?>" data-user="<?= $user_id ?>" data-attendance="<?= $attendance + 1 ?>">+ 1</a> <?php _e( 'or', 'course-booking-system' ); ?> <a href="#" title="<?php _e( 'Decrease the number of participants by 1 for this day only.', 'course-booking-system' ); ?>" class="action-attendance" data-id="<?= $course_id ?>" data-date="<?= $date ?>" data-user="<?= $user_id ?>" data-attendance="<?= $attendance - 1 ?>">- 1</a></small><?php endif; ?></p>
							<?php
						} else {
							?>
							<p class="availability"><?php _e( 'Availability:', 'course-booking-system' ); ?> <strong><?= ( intval( $attendance ) - intval( $attendance_count ) ) ?></strong><?php if ( in_array( reset( $current_user->roles ), $roles ) ) : ?> | <small><a href="#" title="<?php _e( 'Increase the number of participants by 1 for this day only.', 'course-booking-system' ); ?>" class="action-attendance" data-id="<?= $course_id ?>" data-date="<?= $date ?>" data-user="<?= $user_id ?>" data-attendance="<?= $attendance + 1 ?>">+ 1</a> <?php _e( 'or', 'course-booking-system' ); ?> <a href="#" title="<?php _e( 'Decrease the number of participants by 1 for this day only.', 'course-booking-system' ); ?>" class="action-attendance" data-id="<?= $course_id ?>" data-date="<?= $date ?>" data-user="<?= $user_id ?>" data-attendance="<?= $attendance - 1 ?>">- 1</a></small><?php endif; ?></p>
							<?php
						}
						?>
				...
				if ( is_user_logged_in() && $price_level == 5 ) {
					...
					$card_level_name = cbs_get_price_level_name( 5 );
					...
					$card_level_name = cbs_get_price_level_name( 4 );
					...
					$card_level_name = cbs_get_price_level_name( 3 );
					...
					$card_level_name = cbs_get_price_level_name( 2 );
					...
					$card_level_name = cbs_get_price_level_name( 1 );
				...
				<?php } else if ( is_user_logged_in() && !in_array( $user_id, $booked ) && !$free ) { ?>
					...
					<a href="<?= get_permalink( wc_get_page_id( 'shop' ) ) ?>?price-level=<?= $price_level ?>&last-course-visited=<?= urlencode( $last_visited_course ) ?>" class="button btn btn-primary et_pb_button shop-button"><?php _e( 'Shop', 'woocommerce' ); ?></a>
				<?php } else if ( !in_array( $user_id, $booked ) || !is_user_logged_in() ) { ?>
					...
					<a href="<?= get_permalink( get_option( 'woocommerce_myaccount_page_id' ) ).'?message=login&redirect_to='.urlencode( add_query_arg( 'course_id', $course_id, get_permalink( $post_id ) ) ) ?>" class="button btn btn-primary et_pb_button"><?php _e( 'Login', 'woocommerce' ); ?></a>
				<?php }
				...
				if ( is_user_logged_in() && ( current_user_can( 'administrator' ) || current_user_can( 'shop_manager' ) || current_user_can( 'editor' ) || current_user_can( 'author' ) || get_current_user_id() == $course->user_id || ( !empty( $substitute_id ) && get_current_user_id() == $substitute_id ) || $_SERVER['HTTP_HOST'] == 'hundeschule-team-wuff.com' ) && !( $date >= $holiday_start && $date <= $holiday_end ) && !cbs_is_holiday( date( 'd', $i ), date( 'm', $i ), date( 'Y', $i ) ) ) {
					?>
					<h4 class="bookings-headline"><?php _e( 'Bookings', 'woocommerce' ); ?></h4>
					...
					<?php if ( count( $waitlists ) > 0 || $attendance_count >= $attendance ) { ?>
						<h4 class="waitlist-headline"><?php _e( 'Waiting list', 'course-booking-system' ); ?></h4>
						...
						<p class="email"><small><a href="<?= substr( $mailto, 0, -1 ).'&subject='.__( 'Information about the course', 'course-booking-system' ).' '.get_the_title( $post_id ).' '.__( 'on', 'course-booking-system' ).' '.date_i18n( 'l', $weekday ).', '.date_i18n( $date_format, $i ).', '.strip_tags( cbs_get_time_formatted( $course->start, $course->end ) ) ?>"><?php _e( 'Email to all on the waiting list', 'course-booking-system' ); ?></a></small></p>
						<?php
					}
					...
					if ( count( $cancellations ) > 0 ) { ?>
						<h4 class="cancellations-headline"><?php _e( 'Cancellations', 'course-booking-system' ); ?></h4>
		...
		if ( $booking_in_advance == 1 )
			break;
	}

	if ( $course->day != 99 && $booking_in_advance > 1 )
		echo '</div>';
}
