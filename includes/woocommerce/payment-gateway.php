...
// Compatibility with Stripe Payment Methods
...
// Card redemption
function cbs_register_redemption_payment_method_type() {
	if ( !class_exists( 'Automattic\WooCommerce\Blocks\Payments\Integrations\AbstractPaymentMethodType' ) )
		return;

	final class WC_Gateway_Redemption_Blocks extends AbstractPaymentMethodType {

		private $gateway;
		protected $name = 'redemption';

		public function initialize() {
			$this->settings = get_option( 'woocommerce_redemption_settings', [] );
			$this->gateway = new WC_Gateway_Redemption();
		}
		...
		public function get_payment_method_script_handles() {
			...
			wp_register_script(
				'cbs_redemption_gateway-blocks-integration',
				plugin_dir_url( __FILE__ ).'payment-gateway.js',
				[
					'wc-blocks-registry',
					'wc-settings',
					'wp-element',
			...
			if ( function_exists( 'wp_set_script_translations' ) )
				wp_set_script_translations( 'cbs_redemption_gateway-blocks-integration');

			return [ 'cbs_redemption_gateway-blocks-integration' ];
		}
	...
	add_action( 'woocommerce_blocks_payment_method_type_registration', function( Automattic\WooCommerce\Blocks\Payments\PaymentMethodRegistry $payment_method_registry ) {
		$payment_method_registry->register( new WC_Gateway_Redemption_Blocks );
	} );
}
add_action( 'woocommerce_blocks_loaded', 'cbs_register_redemption_payment_method_type' );

function cbs_init_redemption_gateway_class() {
	if ( !class_exists( 'WC_Payment_Gateway' ) )
		return;

	class WC_Gateway_Redemption extends WC_Payment_Gateway {
		...
		public function email_instructions( $order, $sent_to_admin, $plain_text = false ) {
			if ( $this->instructions && !$sent_to_admin && 'redemption' === $order->payment_method )
...
function cbs_redemption_payment_update_order_meta( $order_id ) {
	...
	$order->update_meta_data( 'transaction', $_POST['transaction'] );
	$order->save();
}
...
function cbs_redemption_checkout_field_display_admin_order_meta( $order ) {
	$method = $order->get_meta( '_payment_method', true );
	if ( $method != 'redemption' )
		return;

	echo '<p><strong>'.__( 'Transaction ID' ).':</strong> ' . $order->get_meta( 'transaction', true ) . '</p>';
}
