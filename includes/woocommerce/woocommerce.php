<?php
// Rights for order status paid
function cbs_custom_woocommerce_order_is_paid_statuses( $statuses ) {
	$statuses[] = 'paid';
	return $statuses;
}
add_filter( 'woocommerce_order_is_paid_statuses', 'cbs_custom_woocommerce_order_is_paid_statuses' );
...
function cbs_woocommerce_login_text() {
	if ( !is_user_logged_in() && isset( $_GET['message'] ) ) :
		...
		if ( $_GET['message'] == 'login' )
			wc_add_notice( __( 'Please login first and then try again.', 'course-booking-system' ), 'error' );

		if ( $_GET['message'] == 'delete' )
			wc_add_notice( __( 'The user account has been successfully deleted.', 'course-booking-system' ), 'error' );
	endif;
}
...
function cbs_woocommerce_extra_register_fields() {
	...
	<p class="form-row form-row-first">
		...
		<input type="tel" class="input-text" name="billing_postcode" id="billing_postcode" value="<?php if ( !empty( $_POST['billing_postcode'] ) ) esc_attr_e( $_POST['billing_postcode'] ); ?>" />
	</p>
	<p class="form-row form-row-last">
	...
	<?php } if ( get_option( 'course_booking_system_woocommerce_birthday' ) ) { ?>
		<p class="form-row form-row-wide">
	...
	<?php } if ( get_option( 'course_booking_system_woocommerce_referral' ) ) {
		$limit = get_option( 'course_booking_system_woocommerce_referral_limit' ) ;
		?>
		<p class="form-row form-row-wide">
			<label for="referral"><?= sprintf( __( 'Who referred you? (Enter email and the referrer and you get a free credit, max. %s recommendations)', 'course-booking-system' ), $limit ) ?> <span class="optional">(<?php _e( 'optional', 'woocommerce' ); ?>)</span></label>
...
function cbs_woocommerce_created_customer( $customer_id, $new_customer_data, $password_generated ) {
	...
	if ( !empty( $referral ) ) {
		...
		if ( !empty( $referrer ) ) {
			...
			$referrals = $wpdb->get_results( "SELECT * FROM ".$wpdb->prefix."cbs_logs WHERE action = 'referral (".$referral.")'" );

			// Referrer customer
			if ( !empty( $card ) && $expire > date( 'Y-m-d' ) && count( $referrals ) < $limit ) {
				$card_balance = $card + 1;
			} else {
				$card_balance = 1;
...
function cbs_woocommerce_auto_complete_order( $order_id ) {
	...
	if ( get_option( 'course_booking_system_woocommerce_auto_complete_order' ) ) {
		$order = wc_get_order( $order_id );
		if ( $order->get_status() == 'pending' || $order->get_status() == 'on-hold' || $order->get_status() == 'processing' )
...
function cbs_woocommerce_download_file_redirect( $file_path, $filename ) {
	...
	if ( !empty( $price_level_2 ) && $count > 0 && $product_id == $downloads[0]['product_id'] ) { // Mixed cards
		...
		if ( $count == 1 && $quantity == 1 ) // Upgrade cards
...
add_action( 'woocommerce_download_file_xsendfile', 'cbs_woocommerce_download_file_redirect', 5, 2 );
add_action( 'woocommerce_download_file_redirect', 'cbs_woocommerce_download_file_redirect', 5, 2 );
...
function cbs_wp_die_handler( $message, $title = '', $args = array() ) {
	if ( $message = __( 'Sorry, you have reached your download limit for this file', 'woocommerce' ) ) {
		$account_url = get_permalink( get_option( 'woocommerce_myaccount_page_id' ) );
		wp_redirect( add_query_arg( 'message', 'limit', $account_url ) );
		exit;
	}
...
function cbs_woocommerce_file_download_method() {
	...
	if ( ( $woocommerce_file_download_method != 'xsendfile' && $woocommerce_file_download_method != 'redirect' ) ) {
		?>
		<div class="error">
			<p><?php printf( __( 'The plugin "Course Booking System" uses the <a href="%s">WooCommerce file download method setting</a> to redeem products. Please make sure that the setting is set to "X-Accel-Redirect / X-Sendfile" or "Redirect only (Insecure)". Please also make sure that the setting "Downloads Require Login" is checked.', 'course-booking-system' ), admin_url().'/admin.php?page=wc-settings&tab=products&section=downloadable' ); ?></p>
		</div>
		<?php
	}
	...
	if ( $woocommerce_enable_guest_checkout != 'no' || $woocommerce_enable_checkout_login_reminder != 'yes' ) {
		?>
		<div class="error">
			<p><?php printf( __( 'The plugin "Course Booking System" uses the <a href="%s">WooCommerce user account</a> to redeem products. Please make sure that the setting for "Allow customers to place orders without an account" is not checked and the setting for "Allow customers to log into an existing account during checkout" is checked.', 'course-booking-system' ), admin_url().'/admin.php?page=wc-settings&tab=account' ); ?></p>
		</div>
		<?php
	}
}
...
function cbs_woocommerce_product_archive_description() {
	if ( isset( $_GET['price-level'] ) && !get_option( 'woocommerce_shop_page_display' ) )
		wc_add_notice( __( 'Thank you for your interest in a card for your desired course. The appropriate cards are shown below.', 'course-booking-system' ).' <a href="'.get_permalink( wc_get_page_id( 'shop' ) ).'" tabindex="1" class="button wc-forward">'.__( 'Show all products', 'course-booking-system' ).'</a>', 'notice' );
}
