...
function cbs_register_product_type() {
	class WC_Product_Redeem extends WC_Product {
		...
		public function add_to_cart_url() {
			return apply_filters( 'woocommerce_product_add_to_cart_url', '?add-to-cart='.$this->get_id(), $this );
		}
...
function cbs_woocommerce_js() {
	...
	jQuery( document ).ready( function() {
		// Legacy: Remove (Onload: check if product has redeem attributes to set the right product type)
		...
		// User changes product type to redeem
	...
	<div id="subscription_options" class="panel woocommerce_options_panel">
		<div class="options_group">
			...
			woocommerce_wp_text_input( array(
				...
				'type'			=> 'number',
				'custom_attributes' => array( 'step' => '0.1', 'min' => '1', 'max' => '24' )
			) );
...
add_action( 'woocommerce_redeem_add_to_cart', function() {
...
function cbs_product_meta() {
	...
	if ( !empty( $subscription_expiry ) ) {
		...
		if ( $subscription_expiry < 0.1 ) { // Hack for a subscription for one day
			$duration = sprintf( __( '%s day', 'course-booking-system' ), 1 );
			$expire = '+1 day';
		} else if ( $subscription_expiry == 2.5 ) { // Hack for a subscription for ten weeks
			$duration = sprintf( __( '%s weeks', 'course-booking-system' ), 10 );
			$expire = '+10 weeks';
		} else {
			$duration = sprintf( _n( '%s month', '%s months', $subscription_expiry, 'course-booking-system' ), number_format_i18n( $subscription_expiry ) );
			$expire = '+'.$subscription_expiry.' months -1 day';
		}

		echo '<span class="subscription-expiry">'.__( 'Subscription duration:', 'course-booking-system' ).' '.$duration.' ';

		$subscription_start = get_post_meta( $product->get_id(), '_subscription_start', true );
		if ( !empty( $subscription_start ) )
			echo __( 'from', 'course-booking-system' ).' '.date_i18n( $date_format, strtotime( $subscription_start ) ).' ';

		echo __( 'until', 'course-booking-system' ).' '.date_i18n( $date_format, strtotime( $subscription_start.' '.$expire ) ).'</span>'.PHP_EOL;
	}
	...
	if ( !empty( $video_price_level ) )
		echo '<span class="video-price-level">'.__( 'Card redemption:', 'course-booking-system' ).' '.cbs_get_price_level_name( $video_price_level ).'</span>'.PHP_EOL;
}
