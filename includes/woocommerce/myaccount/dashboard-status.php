...
if ( !empty( $abo ) || $abo_expire >= date( 'Y-m-d' ) ) {
	if ( !empty( $abo_course ) ) {
		...
		$status .= '<p>'.__( 'You have a subscription to the course', 'course-booking-system' ).' "<strong>'.$post_title.', '.cbs_get_weekday( $day ).', '.cbs_get_time_formatted( $start, $end ).'</strong>"';
		...
		if ( !empty( $abo_course_2 ) ) {
			...
			$status .= ' '.__( 'and a subscription to the course', 'course-booking-system' ).' "<strong>'.$post_title_2.', '.cbs_get_weekday( $day_2 ).', '.cbs_get_time_formatted( $start_2, $end_2 ).'</strong>"';
			...
			$status .= ' '.__( 'and a subscription to the course', 'course-booking-system' ).' "<strong>'.$post_title_3.', '.cbs_get_weekday( $day_3 ).', '.cbs_get_time_formatted( $start_3, $end_3 ).'</strong>"';
		}
