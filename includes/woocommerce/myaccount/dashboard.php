...
function cbs_woocommerce_account_dashboard() {
	...
	if ( isset( $_GET['message'] ) && $_GET['message'] == 'purchase' ) :
		if ( isset( $_COOKIE['last-course-visited'] ) && filter_var( $_COOKIE['last-course-visited'], FILTER_VALIDATE_URL ) ) :
			wc_print_notice( __( 'Thank you for your purchase. Your card was successfully redeemed. You can now book appointments.', 'course-booking-system' ).' '.__( 'Go now to your last visited course and continue booking.', 'course-booking-system' ).' <a href="'.htmlspecialchars( $_COOKIE['last-course-visited'] ).'" tabindex="1" class="button wc-forward">» '.__( 'Go to course', 'course-booking-system' ).'</a>', 'success' );
		else :
			wc_print_notice( __( 'Thank you for your purchase. Your card was successfully redeemed. You can now book appointments.', 'course-booking-system' ), 'success' );
		endif;
	elseif ( ( isset( $_GET['message'] ) && $_GET['message'] == 'subscription' ) || ( $abo_expire > date( 'Y-m-d' ) && empty( $abo_course ) ) ) : ?>
		<div class="woocommerce-message subscription">
			...
			if ( $abo_expire > date( 'Y-m-d' ) && empty( $abo_course ) ) {
				...
				foreach ( $courses as $course ) {
					$options[$course->id] = $course->post_title.', '.cbs_get_weekday( $course->day ).', '.cbs_get_time_formatted( $course->start, $course->end );
			...
			<?php } else {
				_e( 'Thank you. You already have a valid subscription that has just been extended.', 'course-booking-system' );
			}
			?>
		</div>
	<?php
	elseif ( isset( $_GET['message'] ) && $_GET['message'] == 'limit' ) :
		wc_print_notice( __( 'Sorry, you have reached your download limit for this file', 'course-booking-system' ), 'notice' );
	endif;
	?>

	<p class="woocommerce-MyAccount-content-cbs-description"><?= sprintf( __( 'You can also view your bookings and cancel bookings in your account. If you cancel your course at least %s hours before the course starts, you will receive a credit.', 'course-booking-system' ), $deleting_in_advance ) ?></p>
	...
	<?php
	$bookings = $wpdb->get_results( "SELECT booking_id, course_id, ".$wpdb->prefix."cbs_bookings.date FROM ".$wpdb->prefix."cbs_bookings JOIN ".$wpdb->prefix."cbs_data ON(".$wpdb->prefix."cbs_bookings.course_id = ".$wpdb->prefix."cbs_data.id) WHERE ".$wpdb->prefix."cbs_bookings.user_id = $user_id AND ".$wpdb->prefix."cbs_bookings.date >= CURDATE() ORDER BY ".$wpdb->prefix."cbs_bookings.date, start" );
	if ( count( $bookings ) > 0 ) {
		?>
				...
				foreach ( $bookings as $booking ) {
					...
					foreach ( $courses as $course ) {
						...
						if ( !empty( $invitation_link ) && ( $current_time + 25 * MINUTE_IN_SECONDS ) >= strtotime( $date.' '.$course->start ) && $current_time < strtotime( $date.' '.$course->end ) ) {
							...
							$password = !empty( $invitation_link_password ) ? ' ('.__( 'Password:', 'course-booking-system' ).' '.$invitation_link_password.')' : '';
							wc_print_notice( __( 'Your course starts right away. Click on the following link and take part now:', 'course-booking-system' ).' <a href="'.esc_url( $invitation_link ).'" tabindex="1" class="button wc-forward" target="_blank">'.__( 'Take part now', 'course-booking-system' ).'</a>'.$password, 'notice' );

							wc_print_notice( __( 'Your course starts right away. Click on the following link and take part now:', 'course-booking-system' ).' <a href="'.esc_url( $invitation_link ).'" tabindex="1" class="button wc-forward" target="_blank">'.__( 'Take part now', 'course-booking-system' ).'</a>'.$password, 'notice', array( 'fixed' => 'true' ) );
						...
						echo '<tr id="booking-id-'.$booking_id.'"><td><a href="'.add_query_arg( 'course_id', $course_id, get_permalink( $course->post_id ) ).'">'.$course->post_title.'</a></td><td><a href="'.cbs_get_weekday_permalink( $course->day, $date ).'">';

						if ( $course->day != 99 )
							echo cbs_get_weekday( $course->day ).', ';

						echo date_i18n( $date_format, strtotime( $date ) ).'</a></td><td>'.cbs_get_time_formatted( $course->start, $course->end ).'</td><td>'.$actions.'</td></tr>';
					}
				}
				?>
			</tbody>
	...
	<?php } else if ( empty( $abo ) && $abo_expire < date( 'Y-m-d' ) ) {
		wc_print_notice( __( 'You haven\'t booked any upcoming courses.', 'course-booking-system' ), 'error' );
	}
	...
	if ( !empty( $abo ) || $abo_expire > date( 'Y-m-d' ) ) :
		if ( !empty( $abo_course ) ) :
			...
			$difference_weeks = floor( ( strtotime( $abo_expire ) - strtotime( $abo_start ) ) / 604800 ) + 1;
			for ( $i = 1; $i <= $difference_weeks; $i++ ) {
				...
				if ( cbs_is_holiday( date( 'd', strtotime($date) ), date( 'm', strtotime($date) ), date( 'Y', strtotime($date) ) ) ) {
					...
					$rows[$date.' '.$start] = '<tr id="abo-date-'.$date.'"><td><a href="'.add_query_arg( 'course_id', $abo_course, get_permalink( $post_id ) ).'">'.$post_title.'</a></td><td><a href="'.cbs_get_weekday_permalink( $day, $date ).'">'.cbs_get_weekday( $day ).', '.date_i18n( $date_format, strtotime( $date ) ).'</a></td><td>'.cbs_get_time_formatted( $start, $end ).'</td><td>'.$actions.'</td></tr>';
				}
			}
		endif;
		...
		if ( !empty( $abo_course_2 ) ) :
			...
			for ( $i = 1; $i <= $difference_weeks; $i++ ) {
				...
				if ( cbs_is_holiday( date( 'd', strtotime($date) ), date( 'm', strtotime($date) ), date( 'Y', strtotime($date) ) ) ) {
					...
					$rows[$date.' '.$start_2] = '<tr id="abo-date-'.$date.'"><td><a href="'.add_query_arg( 'course_id', $abo_course_2, get_permalink( $post_id_2 ) ).'">'.$post_title_2.'</a></td><td><a href="'.cbs_get_weekday_permalink( $day_2, $date ).'">'.cbs_get_weekday( $day_2 ).', '.date_i18n( $date_format, strtotime( $date ) ).'</a></td><td>'.cbs_get_time_formatted( $start_2, $end_2 ).'</td><td>'.$actions.'</td></tr>';
				}
			}
		endif;
		...
		if ( !empty( $abo_course_3 ) ) :
			...
			for ( $i = 1; $i <= $difference_weeks; $i++ ) {
				...
				if ( cbs_is_holiday( date( 'd', strtotime($date) ), date( 'm', strtotime($date) ), date( 'Y', strtotime($date) ) ) ) {
					...
					$rows[$date.' '.$start_3] = '<tr id="abo-date-'.$date.'"><td><a href="'.add_query_arg( 'course_id', $abo_course_3, get_permalink( $post_id_3 ) ).'">'.$post_title_3.'</a></td><td><a href="'.cbs_get_weekday_permalink( $day_3, $date ).'">'.cbs_get_weekday( $day_3 ).', '.date_i18n( $date_format, strtotime( $date ) ).'</a></td><td>'.cbs_get_time_formatted( $start_3, $end_3 ).'</td><td>'.$actions.'</td></tr>';
				}
			}
		endif;
	endif;
	...
	$bookings = $wpdb->get_results( "SELECT booking_id, course_id, ".$wpdb->prefix."cbs_bookings.date FROM ".$wpdb->prefix."cbs_bookings WHERE user_id = $user_id AND date < CURDATE() ORDER BY date DESC LIMIT $bookings_past" );
	if ( count( $bookings ) > 0 ) {
		?>
		...
		<table class="bookings-past-table">
			<tbody>
				...
				foreach ( $bookings as $booking ) {
					...
					foreach ( $courses as $course ) {
						echo '<tr><td><a href="'.add_query_arg( 'course_id', $course_id, get_permalink( $course->post_id ) ).'">'.$course->post_title.'</a></td><td><a href="'.cbs_get_weekday_permalink( $course->day, $date ).'">';

						if ( $course->day != 99 )
							echo cbs_get_weekday( $course->day ).', ';

						echo date_i18n( $date_format, strtotime( $date ) ).'</a></td><td>'.cbs_get_time_formatted( $course->start, $course->end ).'</td></tr>';
					}
				}
				?>
			</tbody>
...
function cbs_my_account_endpoint_content() {
 	if ( isset( $_GET['message'] ) && $_GET['message'] == 'purchase' )
 		wc_print_notice( __( 'Thank you for your purchase. Your video was successfully added to your library. You can now watch the purchased video.', 'course-booking-system' ), 'success' );
		...
		foreach ( $downloads as $download ) :
			...
			if ( !empty( $video_url ) && ( strtotime( $video_expiry ) > current_time( 'timestamp' ) || empty($video_expiry) ) ) {
				...
				<?php elseif ( str_contains( $video_url, 'youtube' ) || str_contains( $video_url, 'youtu.be' ) ) : ?>
					...
					?>
					<iframe width="560" height="315" src="https://www.youtube-nocookie.com/embed/<?= $youtube_id ?>&controls=2&color=white&rel=0&modestbranding=1" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; modestbranding; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
				<?php elseif ( str_contains( $video_url, 'vimeo' ) ) : ?>
					...
					wc_print_notice( __( 'The video is only available externally and cannot be played here. To play the video, please follow this link:', 'course-booking-system' ).' <a href="'.$video_url.'" target="_blank" class="button wc-forward">'.__( 'Go to video', 'course-booking-system' ).'</a>', 'notice' );
				endif;
				?>

				<p><?= !empty( $video_expiry ) ? '<strong class="video-expiry">'.sprintf( __( 'The video is available until %s o\'clock.', 'course-booking-system' ), date_i18n( $date_format.' '.$time_format, strtotime( $video_expiry ) ) ).'</strong>' : __( 'The video is available indefinitely.', 'course-booking-system' ) ?><?= !empty( $video_url_password ) ? '<br><em class="video-password">'.__( 'Password:', 'course-booking-system' ).' '.$video_url_password.'</em>' : '' ?></p>
				<?php
			}
		endforeach;
	endif;
	...
	if ( count( $downloads ) == 0 && count( $video_ids ) == 0 )
		wc_print_notice( __( 'You don\'t have any videos available. Please buy a video in our shop.', 'course-booking-system' ).' <a href="'.get_permalink( wc_get_page_id( 'shop' ) ).'" tabindex="1" class="button wc-forward">'.__( 'Go to shop', 'course-booking-system' ).'</a>', 'notice' );
}
