...
function cbs_shortcode_timetable( $atts = array(), $date = '' ) {
	...
	if ( !empty( $opening ) && $opening > date( 'Y-m-d' ) ) :
		$ajax = false;
		$date = $opening;
	elseif ( empty( $date ) && !empty( $atts ) && array_key_exists( 'date', $atts ) ) :
		$ajax = false;
		$date = $atts['date'];
	elseif ( empty( $date ) && $booking_in_advance > 1 ) :
		...
		// On Sundays: Go to next week if no courses are taking place
		$args = array(
			'category' => $category,
			'day'      => 7,
			'date'     => date( 'Y-m-d' )
		);
		if ( date( 'N' ) == 7 && !cbs_has_courses( $args ) )
			$date = date( 'Y-m-d', strtotime( '+1 day' ) );

		$display = '';
		if ( !in_array( reset( $current_user->roles ), $roles ) )
			$display = 'style="display: none;"';

		$content .= '<div class="cbs-pagination">
			<a href="#" class="action-week cbs-week cbs-week-prev" data-category="'.$category.'" data-design="'.$design.'" data-direction="prev" '.$display.'>'.__( 'Previous week', 'course-booking-system' ).'</a>
	...
	for ( $k = 0; $k < count( $daytimes ); $k++ ) {
		$content .= '<div class="cbs-timetable '.$design.' '.$daytimes[$k]['id'].'" data-design="'.$design.'">';
			for ( $i = 1; $i <= 7; $i++ ) {
				...
				if ( $booking_in_advance == 1 && date( 'N' ) > date( 'N', $weekday ) && ( empty( $opening ) || $opening <= date( 'Y-m-d' ) ) ) :
					$day = $date.' next Sunday +'.$i.' days';
				else :
					$day = $i == 7 ? $date.' Sunday -1 week +'.$i.' days' : $date.' last Sunday +'.$i.' days';
				endif;
				...
				if ( !empty( $atts ) && array_key_exists( 'date', $atts ) )
					unset( $args['day'] );
					...
					if ( strtotime( $day ) < strtotime( 'today' ) ) :
						...
						$content .= '<ul class="cbs-timetable-list">';
							...
							foreach ( $courses as $course ) :
								...
								if ( $substitute_id == 99999 || ( date( 'Y-m-d', strtotime( $day ) ) >= $holiday_start && date( 'Y-m-d', strtotime( $day ) ) <= $holiday_end ) || cbs_is_holiday( date( 'd', strtotime( $day ) ), date( 'm', strtotime( $day ) ), date( 'Y', strtotime( $day ) ) ) || ( $course->day != 99 && !empty( $course->date ) && $course->date > date( 'Y-m-d', strtotime( $day ) ) ) ) :
									if ( $show_cancelled ) :
										$disabled = 'disabled';
									else :
										continue;
								...
								$permalink = add_query_arg( array( 'course_id' => $course->id, 'slide' => $slide ), get_permalink( $course->post_id ) );
								...
								if ( has_post_thumbnail( $course->post_id ) ) :
									...
									$additional_content = '';
									ob_start();
									do_action( 'cbs_timetable_before_list_item', $course->id, date( 'Y-m-d', strtotime( $day ) ) );
									...
									$additional_content = '';
									ob_start();
									do_action( 'cbs_timetable_after_list_item_title', $course->id, date( 'Y-m-d', strtotime( $day ) ) );
									$additional_content = ob_get_contents();
									ob_end_clean();
									$content .= $additional_content;

									$attendance = get_post_meta( $course->post_id, 'attendance', true );
									if ( $attendance > 0 && strtotime( $day ) >= strtotime( 'today' ) && ( empty( $course->date ) || strtotime( $course->date ) <= strtotime( $day ) ) && $substitute_id != 99999 && ( empty( $holiday_start ) || date( 'Y-m-d', strtotime( $day ) ) < $holiday_start || date( 'Y-m-d', strtotime( $day ) ) > $holiday_end ) && !cbs_is_holiday( date( 'd', strtotime( $day ) ), date( 'm', strtotime( $day ) ), date( 'Y', strtotime( $day ) ) ) && ( $show_availability || in_array( reset( $current_user->roles ), $roles ) ) )
										$content .= '<p class="attendance" data-id="'.$course->id.'">'. cbs_attendance( $course->id, date( 'Y-m-d', strtotime( $day ) ), false ).'</p>';

									if ( $substitute_id == 99999 || ( date( 'Y-m-d', strtotime( $day ) ) >= $holiday_start && date( 'Y-m-d', strtotime( $day ) ) <= $holiday_end ) || cbs_is_holiday( date( 'd', strtotime( $day ) ), date( 'm', strtotime( $day ) ), date( 'Y', strtotime( $day ) ) ) || ( !empty( $course->date ) && strtotime( $course->date ) > strtotime( $day ) ) ) :
									...
									$additional_content = '';
									ob_start();
									do_action( 'cbs_timetable_after_list_item', $course->id, date( 'Y-m-d', strtotime( $day ) ) );
									$additional_content = ob_get_contents();
									ob_end_clean();
									$content .= $additional_content;
	...
	if ( !$ajax && !array_key_exists( 'date', $atts ) )
		$content .= '</div>
		<div id="ajax-loader" class="loader"><div></div><div></div><div></div></div>
		<div id="booking-in-advance" data-id="'.$booking_in_advance.'"></div>';

	return $content;
}

function cbs_shortcode_preview( $atts = array() ) {
	global $wpdb;
	...
	$category = ( !empty( $atts ) && array_key_exists( 'category', $atts ) ) ? $atts['category'] : '';
	$category = ( is_array( $category ) ) ? implode( ',', $category ) : $category; // Blocks
	$months = ( !empty( $atts ) && array_key_exists( 'months', $atts ) ) ? $atts['months'] : 6; // Shortcode or default option
	...
	$content .= '<div class="cbs-timetable preview">';
		for ( $i = 0; $i < $months; $i++ ) {
			if ( !empty( $courses ) ) :
				$content .= '<div class="cbs-timetable-column">';
					$content .= '<h4>'.date_i18n( 'F', strtotime( '+'.$i.' months' ) ).'</h4>';
					$content .= '<ul class="cbs-timetable-list">';
						foreach ( $courses as $course ) :
							if ( empty( $course->date ) || date( 'n', strtotime( $course->date ) ) != date( 'n', strtotime( '+'.$i.' months' ) ) )
								continue;

							$user = get_userdata( $course->user_id );

							$permalink = add_query_arg( 'course_id', $course->id, get_permalink( $course->post_id ) );
							$timetable_custom_url = get_post_meta( $course->post_id, 'timetable_custom_url', true );
							if ( !empty( $timetable_custom_url ) )
								$permalink = $timetable_custom_url;

							if ( has_post_thumbnail( $course->post_id ) ) :
								$content .= '<li class="cbs-timetable-list-item"><a title="'.$course->post_title.'" href="'.$permalink.'" style="background-color: '.get_post_meta( $course->post_id, 'color', true ).'; color: '.get_post_meta( $course->post_id, 'text_color', true ).'; background-image: url('.get_the_post_thumbnail_url( $course->post_id ).');">
									<div class="overlay" style="background-color: '.get_post_meta( $course->post_id, 'color', true ).'"></div>';
							else :
								$content .= '<li class="cbs-timetable-list-item"><a title="'.$course->post_title.'" href="'.$permalink.'" style="background-color: '.get_post_meta( $course->post_id, 'color', true ).'; color: '.get_post_meta( $course->post_id, 'text_color', true ).';">';
							endif;
								...
								$content .= '<p class="date">
									<time datetime="'.date( 'Y-m-d', strtotime( $course->date ) ).'" class="timeslot-start">'.date_i18n( $date_format, strtotime( $course->date ) ).'</time>
								</p>';
								$content .= '<p class="timeslot">
									<time datetime="'.date( 'H:i', strtotime( $course->start ) ).'" class="timeslot-start">'.date( $time_format, strtotime( $course->start ) ).'</time>
									<span class="timeslot-delimiter"> - </span>
								...
								$attendance = get_post_meta( $course->post_id, 'attendance', true );
								if ( $attendance > 0 && ( $show_availability || in_array( reset( $current_user->roles ), $roles ) ) )
									$content .= '<p class="attendance" data-id="'.$course->id.'">'. cbs_attendance( $course->id, $course->date, false ).'</p>';
...
function cbs_shortcode_statistics( $atts ) {
	...
	if ( is_user_logged_in() && current_user_can( 'administrator' ) ) :
		...
		$user_info = '';
		$users = get_users( [ 'role__in' => cbs_get_roles() ] );
		foreach ( $users AS $user ) {
			$hours = 0;
			$list  = '<ul>';
			$user_id = $user->ID;
...
function cbs_add_shortcodes() {
	...
	add_shortcode( 'preview', 'cbs_shortcode_preview' );
