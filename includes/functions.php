...
function cbs_is_licensed() {
	...
	if ( $response == 'true' || str_contains( $_SERVER['HTTP_HOST'], 'tastewp.com' ) )
		return true;

	return false;
}

function cbs_get_roles() {
	$roles = array( 'administrator', 'shop_manager', 'editor', 'author', 'contributor' );
	return $roles;
}

function cbs_get_price_level_name( $price_level = 1 ) {
	$price_level_names = array(
		1 => get_option( 'course_booking_system_price_level_title' ),
		2 => get_option( 'course_booking_system_price_level_title_2' ),
		3 => get_option( 'course_booking_system_price_level_title_3' ),
		4 => get_option( 'course_booking_system_price_level_title_4' ),
		5 => get_option( 'course_booking_system_price_level_title_5' )
	);

	return $price_level_names[$price_level];
}

function cbs_get_time_formatted( $start, $end = '' ) {
	$time_format = get_option( 'time_format' );

	$time_formatted = '<time datetime="'.date( 'H:i', strtotime( $start ) ).'" class="timeslot-start">'.date( $time_format, strtotime( $start ) ).'</time>';

	if ( !empty( $end ) )
		$time_formatted .= ' - <time datetime="'.date( 'H:i', strtotime( $end ) ).'" class="timeslot-end">'.date( $time_format, strtotime( $end ) ).'</time>';

	if ( !str_contains( ' A', $time_format ) )
		$time_formatted .= ' <span class="time-formatted-suffix">'.__( 'o\'clock', 'course-booking-system' ).'</span>';

	return $time_formatted;
}

function cbs_is_holiday( $day, $month, $year ) {
	if ( strlen( $day ) == 1 ) // Always use standard format with two numbers for days and months
		$day = "0$day";
	...
	if ( get_option( 'course_booking_system_holiday_womens_day' ) )
		$holidays[] = '08.03.'; // International Women's Day
...
function cbs_get_courses( $args = array() ) {
	...
	if ( empty( $args ) ) :
		$courses = $wpdb->get_results( "SELECT ".$wpdb->prefix."cbs_data.id AS id, post_id, post_title, day, date, start, end, user_id FROM ".$wpdb->prefix."posts JOIN ".$wpdb->prefix."cbs_data ON ".$wpdb->prefix."posts.ID = ".$wpdb->prefix."cbs_data.post_id WHERE ".$post_status." ORDER BY day,start" );
		return $courses;
	endif;
...
function cbs_compile_post_type_capabilities( $singular = 'post', $plural = 'posts' ) {
	return [
