...
add_filter( 'manage_course_posts_columns', 'cbs_manage_course_posts_columns' );

function cbs_manage_course_posts_custom_column( $column_name, $post_id ) {
	switch( $column_name ) {
		case 'price_level': {
			$free = get_post_meta( $post_id, 'free', true );
			$price_level = get_post_meta( $post_id, 'price_level', true );
			echo '<span data-free="'.esc_attr( $free ).'" data-price-level="'.esc_attr( $price_level ).'">';
			echo $free ? __( 'Free', 'course-booking-system' ) : cbs_get_price_level_name( $price_level );
			echo '</span>';
			break;
		} case 'attendance': {
			echo get_post_meta( $post_id, 'attendance', true );
			break;
		}
	}

}
add_action( 'manage_course_posts_custom_column', 'cbs_manage_course_posts_custom_column', 10, 2 );

// Quick Edit
function cbs_quick_edit_fields( $column_name, $post_type ) {
	if ( $post_type != 'course' )
		return;
	...
	switch( $column_name ) {
		case 'price_level': {
			?>
			<fieldset class="inline-edit-col-left inline-edit-course-options">
				<div class="inline-edit-col">
					<h4 class="title inline-edit-course-options-label"><?php _e( 'Course Options', 'course-booking-system' ); ?></h4>
					<div class="inline-edit-group wp-clearfix">
						<label class="inline-edit-free">
							<input type="checkbox" name="free" value="1"> <span class="checkbox-title"><?php _e( 'This course is free of charge', 'course-booking-system' ); ?></span>
						</label>
						<label class="inline-edit-price_level">
							<span class="title"><?php _e( 'Price Level', 'course-booking-system' ); ?></span>
							<input type="range" name="price_level" value="0" min="0" max="5">
							<output></output>
						</label>
					</div>
			<?php
			break;
				...
				?>
					<div class="inline-edit-group wp-clearfix">
						<label class="inline-edit-attendance">
							<span class="title"><?php _e( 'Number of participants', 'course-booking-system' ); ?></span>
...
add_action( 'quick_edit_custom_box', 'cbs_quick_edit_fields', 10, 2 );
add_action( 'bulk_edit_custom_box', 'cbs_quick_edit_fields', 10, 2 );
...
function cbs_save_postmeta( $post_id, $post, $update ) {
	...
	if ( isset( $_REQUEST['attendance'] ) && intval( $_REQUEST['attendance'] ) > 0 )
		update_post_meta( $post_id, 'attendance', intval( $_REQUEST['attendance'] ) );

	if ( isset( $_REQUEST['free'] ) && intval( $_REQUEST['free'] ) > 0 ) :
		update_post_meta( $post_id, 'free', intval( $_REQUEST['free'] ) );
	else :
		delete_post_meta( $post_id, 'free' );
	endif;

	if ( isset( $_REQUEST['price_level'] ) && intval( $_REQUEST['price_level'] ) > 0 )
		update_post_meta( $post_id, 'price_level', intval( $_REQUEST['price_level'] ) );

	if ( isset( $_REQUEST['invitation_link'] ) )
		update_post_meta( $post_id, 'invitation_link', sanitize_text_field( $_REQUEST['invitation_link'] ) );

	if ( isset( $_REQUEST['invitation_link_password'] ) )
		update_post_meta( $post_id, 'invitation_link_password', sanitize_text_field( $_REQUEST['invitation_link_password'] ) );

	if ( isset( $_REQUEST['color'] ) )
		update_post_meta( $post_id, 'color', sanitize_text_field( $_REQUEST['color'] ) );

	if ( isset( $_REQUEST['text_color'] ) )
		update_post_meta( $post_id, 'text_color', sanitize_text_field( $_REQUEST['text_color'] ) );

	if ( isset( $_REQUEST['timetable_custom_url'] ) )
		update_post_meta( $post_id, 'timetable_custom_url', sanitize_text_field( $_REQUEST['timetable_custom_url'] ) );
...
function cbs_data_options_table( $post_id ) {
	...
	$table_name = $wpdb->prefix.'cbs_data';
