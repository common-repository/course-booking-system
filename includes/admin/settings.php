...
function cbs_register_settings() {
	...
	add_option( 'course_booking_system_holiday_womens_day', 0 );
	register_setting( 'course_booking_system_options_group_holiday', 'course_booking_system_holiday_womens_day', 'course_booking_system_callback' );
	...
	add_option( 'course_booking_system_woocommerce_referral_limit', 3 );
	register_setting( 'course_booking_system_options_group_woocommerce', 'course_booking_system_woocommerce_referral_limit', 'course_booking_system_callback' );
	...
	add_option( 'course_booking_system_email_waitlist_subject', __( 'A place has become available for the course on', 'course-booking-system' ) );
				...
				<tr>
					...
					<td>
						<select name="course_booking_system_design" id="course_booking_system_design">
							...
							<option value="divided" <?= get_option( 'course_booking_system_design' ) == 'divided' ? 'selected="selected"' : '' ?> <?= !$is_licensed ? 'disabled' : '' ?>><?php _e( 'Divided', 'course-booking-system' ); ?></option>
							<option value="list" <?= ( get_option( 'course_booking_system_design' ) == 'list' ) ? 'selected="selected"' : '' ?> <?= !$is_licensed ? 'disabled' : '' ?>><?php _e( 'List', 'course-booking-system' ); ?></option>
						</select>
						<?php
						if ( !$is_licensed )
			...
			<tbody>
				<tr>
					...
					<td>
						...
						<input type="checkbox" name="course_booking_system_holiday_womens_day" id="course_booking_system_holiday_womens_day" value="1" <?= get_option( 'course_booking_system_holiday_womens_day' ) ? 'checked="checked"' : '' ?> /><?php _e( 'International Women\'s Day', 'course-booking-system' ); ?><br />
		...
		<table class="form-table" role="presentation">
			<tbody>
				...
				<tr>
					...
					<td>
						<input type="checkbox" name="course_booking_system_woocommerce_referral" id="course_booking_system_woocommerce_referral" value="1" <?= get_option( 'course_booking_system_woocommerce_referral' ) ? 'checked="checked"' : '' ?> /><?php _e( 'Give customers the opportunity to refer friends and acquaintances. The new customer and the referrer receive a free credit.', 'course-booking-system' ); ?><br />
					</td>
				</tr>
				...
				<tr id="tr-referral_limit" <?= get_option( 'course_booking_system_woocommerce_referral' ) ? 'style="display: table-row;"' : 'style="display: none;"' ?>>
					<th><label for="course_booking_system_woocommerce_referral_limit"><?php _e( 'Limit of possible referrals', 'course-booking-system' ); ?></label></th>
					<td>
						<input name="course_booking_system_woocommerce_referral_limit" id="course_booking_system_woocommerce_referral_limit" type="number" min="1" value="<?= get_option( 'course_booking_system_woocommerce_referral_limit' ) ?>" class="regular-text">
					</td>
				</tr>
			</tbody>
		</table>
			...
			<tbody>
				<tr>
					...
					<td>
						...
						<p class="description"><?php _e( 'In the event of reversion, the e-mail will be sent to the course participant. No email will be sent if the customer reverses a course. Emails will also be sent to the course instructor for courses with 2 or fewer maximum participants.', 'course-booking-system' ); ?></p>
					</td>
				</tr>
				<tr>
...
function cbs_import_render_options_page() {
	...
	global $wpdb;
	if ( $_SERVER['REQUEST_METHOD'] === 'POST' && isset( $_FILES ) && $_FILES['course_booking_system_import']['error'] != 4 && $_FILES['course_booking_system_import']['size'] > 0 ) {
		$count = 0;
		$skipped = 0;
		...
		while ( ( $data = fgetcsv( $handle, 1000, ',' ) ) !== FALSE ) {
			if ( is_numeric( $data[0] ) ) {
				...
				$post = get_page_by_title( $data[0], OBJECT, 'course' );
				$course_id = $post->ID;
			}
			$date = $data[1];
			$user_id = $data[2];
