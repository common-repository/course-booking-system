...
add_action( 'cbs_cron_quarterly', 'cbs_auto_cancel' );
...
function cbs_abo_control() {
	...
	foreach ( $users as $user ) {
		...
		$abo_2           = get_the_author_meta( 'abo_2', $user_id );
		$abo_3           = get_the_author_meta( 'abo_3', $user_id );
		...
		if ( ( $abo || $abo_2 || $abo_3 ) && date( 'Y-m-d', strtotime( $abo_expire_date.' -'.$abo_period.' weeks' ) ) < date( 'Y-m-d' ) ) { // Extend subscription
		...
		} else if ( !$abo && date( 'Y-m-d', strtotime( $abo_expire_date ) ) < date( 'Y-m-d', strtotime( '-3 months' ) ) ) { // Clean database
...
function cbs_clean_database() {
	...
	$timestamp = date( 'Y-m-d', strtotime( '-12 months' ) );
	$wpdb->get_results( "DELETE FROM ".$wpdb->prefix."cbs_logs WHERE timestamp < '$timestamp'" );
}
