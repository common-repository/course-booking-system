...
function cbs_action_booking( $custom_user_id ) {
	...
	if ( $free || ( $flat && date( 'Y-m-d', strtotime( $flat_expire ) ) >= $date ) ) {
		if ( is_user_logged_in() && !empty( $user_id ) && ( get_current_user_id() == $user_id || !empty( $custom_user_id ) || in_array( reset( $current_user->roles ), $roles ) ) ) {
			$wpdb->insert(
			...
			if ( empty( $custom_user_id ) ) {
				...
				wc_print_notice( __( 'Thank you for booking the course.', 'course-booking-system' ).' <a href="'.get_permalink( $woocommerce_myaccount_page_id ).'" tabindex="1" class="button wc-forward">» '.get_the_title( $woocommerce_myaccount_page_id ).'</a>', 'success' );
			} else {
				return TRUE;
			}
		...
		if ( is_user_logged_in() && !empty( $user_id ) && ( get_current_user_id() == $user_id || !empty( $custom_user_id ) || in_array( reset( $current_user->roles ), $roles ) ) ) {
			if ( $card > 0 && $expire >= $date ) {
				...
				wc_print_notice( __( 'No valid card available.', 'course-booking-system' ), 'error' );
			}
		}
	}
...
function cbs_action_booking_delete() {
	...
	$goodwill = ( !empty( $_REQUEST['goodwill'] ) ) ? sanitize_text_field( $_REQUEST['goodwill'] ) : false;
	...
	if ( is_user_logged_in() && !empty( $user_id ) && ( get_current_user_id() == $user_id || in_array( reset( $current_user->roles ), $roles ) ) ) {
		if ( $goodwill || ( !$free && !( $flat && date( 'Y-m-d', strtotime($flat_expire) ) >= $date ) && ( strtotime( $date.' '.$start ) - $deleting_in_advance * HOUR_IN_SECONDS ) > current_time( 'timestamp' ) ) ) {
			...
			if ( !empty( $abo_expire ) && $abo_expire > $expire ) { // Set subscription expire date if available
				update_user_meta( $user_id, $expire_name, $abo_expire );
			} else if ( empty( $expire ) || $expire < $date ) { // Fallback if course was booked with a higher card and the new price level card has no expiry date yet
			...
			wc_print_notice( __( 'The booking was reversed successfully. A credit has been added to the card.', 'course-booking-system' ), 'notice' );
			...
			wc_print_notice( __( 'Thank you for canceling the course.', 'course-booking-system' ), 'error' );
	...
	if ( $email_deleting && get_current_user_id() != $user_id && ( in_array( reset( $current_user->roles ), $roles ) ) ) {
		$user_info = get_userdata( $user_id );
...
function cbs_action_abo_delete() {
	...
	if ( is_user_logged_in() && !empty( $user_id ) && ( get_current_user_id() == $user_id || in_array( reset( $current_user->roles ), $roles ) ) ) {
		...
		if ( $goodwill || ( !$free && ( strtotime( $date.' '.$start ) - $deleting_in_advance * HOUR_IN_SECONDS ) > current_time( 'timestamp' ) && ( empty( $abo_alternate_option ) || count( $abo_alternate ) <= $abo_alternate_option ) ) ) {
			wc_print_notice( __( 'The booking was reversed successfully. A credit has been added to the card.', 'course-booking-system' ), 'notice' );

			if ( $_SERVER['HTTP_HOST'] != 'studio-a-pilates.de' ) {
				$card++;
				update_user_meta( $user_id, $card_name, $card );
				cbs_log( $user_id, $card_name, $card, $course_id, __FUNCTION__ );

				if ( empty( $expire ) || $expire < date( 'Y-m-d' ) || $expire < $abo_expire )
					update_user_meta( $user_id, $expire_name, $abo_expire );
			}
		} else {
			wc_print_notice( __( 'Thank you for canceling the course.', 'course-booking-system' ), 'error' );
...
function cbs_action_waitlist() {
	...
	if ( is_user_logged_in() && !empty( $user_id ) && ( get_current_user_id() == $user_id || in_array( reset( $current_user->roles ), $roles ) ) ) {
		...
		$woocommerce_myaccount_page_id = get_option( 'woocommerce_myaccount_page_id' );
		wc_print_notice( __( 'Thank you for your interest in this course. You have successfully registered on the waiting list. As soon as a place is available you will be notified by email.', 'course-booking-system' ).' <a href="'.get_permalink( $woocommerce_myaccount_page_id ).'" tabindex="1" class="button wc-forward">» '.get_the_title( $woocommerce_myaccount_page_id ).'</a>', 'notice' );
	}
...
function cbs_action_waitlist_delete() {
	...
	if ( is_user_logged_in() && !empty( $user_id ) && ( get_current_user_id() == $user_id || in_array( reset( $current_user->roles ), $roles ) ) ) {
		...
		if ( $waitlists > 0 ) {
			wc_print_notice( __( 'Thank you for unsubscribing from the waiting list.', 'course-booking-system' ), 'error' );
		}
	}
...
function cbs_action_substitute() {
	...
	if ( is_user_logged_in() && !empty( $user_id ) && in_array( reset( $current_user->roles ), $roles ) ) {
		$wpdb->delete(
		...
		wc_print_notice( __( 'Trainer saved successfully.', 'course-booking-system' ), 'success' );
	}
...
function cbs_action_attendance() {
	...
	if ( is_user_logged_in() && in_array( reset( $current_user->roles ), $roles ) ) {
		$wpdb->delete(
		...
		foreach ( $courses as $course ) :
			$custom_attendance = get_post_meta( $course->post_id, 'attendance', true );
		endforeach;
		...
		wc_print_notice( __( 'The attendance was updated successfully.', 'course-booking-system' ), 'success' );
	}
...
function cbs_note() {
	...
	if ( is_user_logged_in() && ( in_array( reset( $current_user->roles ), $roles ) ) ) {
		...
		wc_print_notice( __( 'Note saved successfully.', 'course-booking-system' ), 'success' );
	}
...
function cbs_action_subscription() {
	...
	if ( is_user_logged_in() ) {
		...
		wc_print_notice( __( 'Subscription course saved successfully.', 'course-booking-system' ), 'success' );
	}
