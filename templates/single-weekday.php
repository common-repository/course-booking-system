...
<div class="container section-inner">
	...
	elseif ( is_user_logged_in() && ( in_array( reset( $current_user->roles ), $roles ) ) ) :
	...
	<div class="cbs-weekday-grid">
		...
		foreach ( $courses as $course ) :
			...
			<div class="cbs-weekday-course course post-<?= $course->post_id ?>">
				<h2><a href="<?= add_query_arg( 'course_id', $course->id, get_permalink( $course->post_id ) ) ?>">
					<?= get_the_title( $course->post_id ) ?>, <?= cbs_get_time_formatted( $course->start, $course->end ) ?>
				</a></h2>

				<h4><?= $free ? __( 'Free', 'woocommerce' ) : cbs_get_price_level_name( $price_level ) ?></h4>
				...
				foreach ( $abos as $abo ) {
					...
					if ( $abo_start <= $date && $abo_expire >= $date && !in_array( $date, $abo_alternate ) ) {
						...
						$table .= '<tr><td>'.$attendance_count.'</td><td class="subscription"><a href="'.get_edit_user_link( $abo->user_id ).'">'.$user_info->first_name.' '.$user_info->last_name.'</a> '.$description.'</td><td><a href="#" class="button btn btn-primary et_pb_button action-abo-delete" title="'.__( 'Reverse', 'course-booking-system' ).'" data-id="'.$course->id.'" data-date="'.$date.'" data-user="'.$abo->user_id.'">×</a></td></tr>';
					}
				}
				...
				foreach ( $bookings as $booking ) {
					...
					$table .= '<tr>';
						...
						$table .= '<td><a href="#" class="button btn btn-primary et_pb_button action-booking-delete" title="'.__( 'Reverse', 'course-booking-system' ).'" data-id="'.$course->id.'" data-date="'.$date.'" data-user="'.$booking->user_id.'" data-booking="'.$booking->booking_id.'">×</a></td>';
					$table .= '</tr>';
				}
				...
				if ( ( $show_availability || in_array( reset( $current_user->roles ), $roles ) ) && !( $date >= $holiday_start && $date <= $holiday_end ) && !cbs_is_holiday( date( 'd', strtotime($date) ), date( 'm', strtotime($date) ), date( 'Y', strtotime($date) ) ) ) {
				...
				<?php if ( is_user_logged_in() && ( current_user_can( 'administrator' ) || current_user_can( 'shop_manager' ) || current_user_can( 'editor' ) || current_user_can( 'author' ) || get_current_user_id() == $course->user_id || ( !empty( $substitute_id ) && get_current_user_id() == $substitute_id ) || $_SERVER['HTTP_HOST'] == 'hundeschule-team-wuff.com' ) && !( $date >= $holiday_start && $date <= $holiday_end ) && !cbs_is_holiday( date( 'd', strtotime($date) ), date( 'm', strtotime($date) ), date( 'Y', strtotime($date) ) ) ) { ?>
					<h3 class="bookings-headline"><?php _e( 'Bookings', 'woocommerce' ); ?></h3>
					...
					if ( count( $waitlists ) > 0 || $attendance_count >= $attendance ) {
						?>
						<h3 class="waitlist-headline"><?php _e( 'Waiting list', 'course-booking-system' ); ?></h3>
					...
					if ( count( $cancellations ) > 0 ) {
						?>
						<h3 class="cancellations-headline"><?php _e( 'Cancellations', 'course-booking-system' ); ?></h3>
	...
	<?php
	if ( ( in_array( reset( $current_user->roles ), $roles ) ) && $attendance_all > 0 ) :
		?>
