 ...
 * Plugin URI: https://commotion.online/en/individual-course-and-online-booking-system/
...
class course_booking_system {
	...
	public static function include_all() {
		...
		require_once plugin_dir_path( __FILE__ ) . 'block/preview/index.php';
	...
	public function livesearch() {
		...
		if ( ( is_singular( 'course' ) || is_post_type_archive( 'course' ) ) && ( in_array( reset( $current_user->roles ), $roles ) ) ) :
			// Variables
			...
			<ul id="livesearch" class="livesearch">
				...
				foreach ( $users AS $user ) :
					...
					echo '<li class="user-'.$user->ID.'" data-user="'.$user->ID.'">';
						...
						if ( is_post_type_archive( 'course' ) ) {
							echo '<a href="#" class="action-booking" data-id="'.$course_id.'" data-date="" data-user="'.$user->ID.'">'.$user_info->first_name.' '.$user_info->last_name.'</a>';
						} else if ( $free ) {
							echo '<a href="#" class="action-booking" data-id="'.$course_id.'" data-date="" data-user="'.$user->ID.'">'.esc_html( $user_info->first_name ).' '.esc_html( $user_info->last_name ).'</a>';
						} else if ( $flat /* && $flat_expire >= $date */ ) {
							echo '<a href="#" class="action-booking" data-id="'.$course_id.'" data-date="" data-user="'.$user->ID.'" data-email="'.esc_html( $user->user_email ).'">'.esc_html( $user_info->first_name ).' '.esc_html( $user_info->last_name ).'<br>(<strong>'.__( 'Flatrate', 'course-booking-system' ).'</strong><span class="expiry"> '.__( 'valid until', 'course-booking-system' ).' '.date_i18n( $date_format, strtotime( $flat_expire ) ).'</span>)</a>';
						} else if ( $card > 0 /* && $expire >= $date */ ) {
							echo '<a href="#" class="action-booking" data-id="'.$course_id.'" data-date="" data-user="'.$user->ID.'" data-email="'.esc_html( $user->user_email ).'">'.esc_html( $user_info->first_name ).' '.esc_html( $user_info->last_name ).'<br>(<strong>'.$card.'</strong><span class="expiry"> '.__( 'valid until', 'course-booking-system' ).' '.date_i18n( $date_format, strtotime( $expire ) ).'</span>)</a>';
						} else {
							echo '<a href="#">'.esc_html( $user_info->first_name ).' '.esc_html( $user_info->last_name ).' - '.__( 'No valid card available.', 'course-booking-system' ).'</a>';
						}
					echo '</li>';
					?>
				<?php endforeach; ?>
			...
			<ul id="livesearch-waitlist" class="livesearch">
				...
				foreach ( $users AS $user ) :
					$user_info = get_userdata( $user->ID );
					echo '<li class="user-'.$user->ID.'" data-user="'.$user->ID.'"><a href="#" class="action-waitlist" data-id="'.$course_id.'" data-date="" data-user="'.$user->ID.'">'.esc_html( $user_info->first_name ).' '.esc_html( $user_info->last_name ).'</a></li>';
				endforeach; ?>
